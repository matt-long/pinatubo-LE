...
for compset in B20TRC5CNBDRD BRCP85C5CNBDRD; do
  ...
  for caseid in {001..035}; do
    ...
    for varname in DIC DIC_ALT_CO2 TEMP O2; do
      ...
      LENS_file=`ls -1 ${in_dir}/b.e11.${compset}.f09_g16.${caseid}.pop.h.${varname}.${LENS_glob}.nc`
  ...
  for caseid in {001..035}; do
    for varname in DIC DIC_ALT_CO2 TEMP O2; do
      ...
      file_in=/glade/campaign/univ/udeo0005/cesmLE_no_pinatubo/ocn/proc/tseries/monthly/${varname}/b.e11.${compset}_no_pinatubo.f09_g16.${caseid}.pop.h.${varname}.${our_years}.nc
      if [ ! -e $file_in ]; then
        continue
      fi
      out_dir=/glade/campaign/univ/udeo0005/cesmLE_no_pinatubo/ocn/proc/zonal_avg_tseries/${varname}
      mkdir -p ${out_dir}
      file_out=${out_dir}/b.e11.${compset}_no_pinatubo.f09_g16.${caseid}.pop.h.${varname}_zavg.${our_years}.nc
      if [ -e $file_out ]; then
        continue
      fi
      subcount=$((subcount+1))
      ourcount=$((ourcount+1))
      submit_job ${job_name}_no_pinatubo ${file_in} ${file_out}
    done
  done
done
...
echo "${ourcount} call(s) to za for no_pinatubo runs"
